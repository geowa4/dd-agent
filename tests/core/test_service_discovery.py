...
class TestServiceDiscovery(unittest.TestCase):
    ...
    def test_get_host_addresses(self, mock_check_yaml, mock_get):
        ...
        # (inspect, expected_ips)
        ip_address_inspects = [
            ({'NetworkSettings': {}}, None),
            ({'NetworkSettings': {'IPAddress': ''}}, None),

            ({'NetworkSettings': {'IPAddress': '127.0.0.1'}}, '127.0.0.1'),
            ({'NetworkSettings': {'IPAddress': '127.0.0.1', 'Networks': {}}}, '127.0.0.1'),
            ({'NetworkSettings': {
                'IPAddress': '127.0.0.1',
            ...
            ({'NetworkSettings': {'Networks': {'bridge': {}}}}, None),
            ({'NetworkSettings': {
                'Networks': {
                    'bridge': {'IPAddress': '172.17.0.2'}
                }}},
            ...
            with mock.patch.object(AbstractConfigStore, '__init__', return_value=None):
                with mock.patch('utils.dockerutil.DockerUtil.client', return_value=None):
                    with mock.patch('utils.kubeutil.get_conf_path', return_value=None):
                        ...
                        self.assertEquals(sd_backend._get_host_addresses(c_ins), expected_ip)
        ...
        with mock.patch('utils.dockerutil.DockerUtil.client', return_value=None):
            for c_ins, _, expected_ports in self.container_inspects:
                ...
                if isinstance(expected_ports, list):
                    self.assertEquals(sd_backend._get_ports(c_ins), expected_ports)
                else:
    ...
    def test_get_check_configs(self, mock_inspect_container, mock_get_conf_tpls):
        ...
        with mock.patch('utils.dockerutil.DockerUtil.client', return_value=None):
            with mock.patch.object(SDDockerBackend, '_get_host_addresses', return_value='127.0.0.1'):
    ...
    def test_fill_tpl(self):
        ...
        edge_cases = [
            ...
            # specify bridge but there is a default IPAddress
            (
                ({'NetworkSettings': {
                    'IPAddress': '127.0.0.1',
                    'Networks': {'bridge': {'IPAddress': '172.17.0.2'}}}},
            ...
            # specify index but there is a default IPAddress
            (
                ({'NetworkSettings': {
                    'IPAddress': '127.0.0.1',
                    'Networks': {'bridge': {'IPAddress': '172.17.0.2'}}}},
                 {'host': '%%host_0%%', 'port': 1337}, ['host_0'], ['foo', 'bar:baz']),
                ({'host': '%%host_0%%', 'port': 1337, 'tags': ['foo', 'bar:baz']}, {'host_0': '127.0.0.1'}),
            ),
            # missing key for host
            (
                ({'NetworkSettings': {'Networks': {
                    'bridge': {'IPAddress': '127.0.0.1'},
                    'foo': {'IPAddress': '172.17.0.2'}}}},
                 {'host': '%%host_bar%%', 'port': 1337}, ['host_bar'], []),
                ({'host': '%%host_bar%%', 'port': 1337}, {'host_bar': '172.17.0.2'}),
            ),
    ...
    def test_process_result_list(self):
        """Test _process_result_list"""
        valid_lists = [
            (['1', '2', '3'], ['port', '0'], '1'),
            (['1', '2', '3'], ['port', 2], '3'),
            (['1', '2', '3'], ['port'], '3'),
                ...
                with mock.patch.object(ConsulStore, 'get_client', return_value=None):
                    for ac in self.agentConfigs:
                        ...
                        for li in valid_lists:
                            res, var_parts, expected_result = li
                            self.assertEquals(
                                sd_backend._process_result_list(res, var_parts), expected_result)

                        for li in invalid_lists:
                            res, var_parts, expected_exception = li
                            with self.assertRaises(expected_exception):
                                sd_backend._process_result_list(res, var_parts)

    def test_process_result_dict(self):
        """Test _process_result_dict"""
        valid_dicts = [
            ({'foo': 'bar', 'bridge': '127.0.0.1'}, ['var', 'foo'], 'bar'),
            ({'foo': 'bar', 'bridge': '127.0.0.1'}, ['var', 'bridge'], '127.0.0.1'),
            ({'foo': 'bar', 'bridge': '127.0.0.1'}, ['baz'], 'bar')
        ]
        invalid_dicts = [
            ({}, [], IndexError),
            ({}, ['0'], IndexError)
        ...
        with mock.patch('utils.dockerutil.DockerUtil.client', return_value=None):
            with mock.patch.object(EtcdStore, 'get_client', return_value=None):
                with mock.patch.object(ConsulStore, 'get_client', return_value=None):
                    for ac in self.agentConfigs:
                        sd_backend = get_sd_backend(agentConfig=ac)
                        for di in valid_dicts:
                            res, var_parts, expected_result = di
                            self.assertEquals(
                                sd_backend._process_result_dict(res, var_parts), expected_result)

                        for di in invalid_dicts:
                            res, var_parts, expected_exception = di
                            with self.assertRaises(expected_exception):
                                sd_backend._process_result_dict(res, var_parts)
